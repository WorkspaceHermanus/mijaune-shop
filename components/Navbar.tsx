    ...
    <>
      ...
      {drawerOpen && (
        <div className="fixed inset-0 z-50 flex">
          <div className="w-72 bg-site h-full shadow-xl flex flex-col p-6 overflow-y-auto">
            <button onClick={() => setDrawerOpen(false)} className="self-end mb-6 text-ink">
              ...
              {navLinks.map(l => (
                <li key={l.href}>
                  <Link href={l.href} onClick={() => setDrawerOpen(false)}
                    className="text-sm font-semibold tracking-widest text-ink hover:text-mustard transition-colors">
                    {l.label}
                  </Link>
                </li>
              ))}
      ...
      <header role="banner" className="sticky top-0 z-40" style={{ backgroundColor: "#fffdf5" }}>
        {/* Upper row — brand + search + cart */}
        <div className="border-b border-gray-200 px-4 md:px-8 py-3 flex items-center">
          {/* Mobile hamburger */}
          <button onClick={() => setDrawerOpen(true)} className="md:hidden mr-4 text-ink">
          ...
          <div className="flex-1 text-center md:text-left">
            <Link href="/" className="text-base font-bold tracking-widest uppercase text-ink">
              Mijaune Shop
            </Link>
          </div>
          ...
          <div className="flex items-center gap-4">
            {/* Search */}
            <button aria-label="Search" className="hidden md:block text-ink hover:opacity-60">
            ...
            {/* Cart */}
            <Link href="/cart" aria-label="Cart" className="relative text-ink hover:opacity-60">
              ...
              {count > 0 && (
                <span className="absolute -top-2 -right-2 bg-ink text-site text-[9px] w-4 h-4 rounded-full flex items-center justify-center font-bold">
                  {count}
                </span>
              )}
            </Link>
        ...
        <nav className="hidden md:block border-b border-gray-200 px-8" role="navigation">
          <div className="flex items-center justify-between">
            <ul className="flex items-center gap-8 h-10">
              {navLinks.map(l => (
                <li key={l.href}>
                  <Link href={l.href}
                    className="text-xs font-semibold tracking-widest text-ink hover:text-mustard transition-colors">
                    {l.label}
                  </Link>
                </li>
              ))}
            </ul>
            <Link href="/contact" className="text-xs tracking-widest text-ink/60 hover:text-ink transition-colors">
              Kontak
            </Link>
          </div>
        </nav>
